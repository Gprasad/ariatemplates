...
Aria.classDefinition({
    ...
    $constructor : function (cfg, ctxt, lineNumber, controller) {
        // allow the skinnable class to be defined in the child class,
        // before
        ...
        this._isMultiAutocomplete = false;
    ...
    $prototype : {
        ...
        _getTextFieldColor : function () {
            ...
            if (this._isIE7OrLess) {
                // As "inherit" does not work in this case in IE7, we
                // are obliged to read the property from
                // the frame state and apply it to the text field
                // directly
        ...
        _getText : function () {
            ...
            if (text && cfg.value) {
                // There's both a value and an invalid text, prefer the
                // value
            ...
            if (res.report) {
                ...
                if (!text && report.text != null) {
                    text = '' + report.text; // String cast of valid
                    // value
                }
                report.$dispose();
            }
        ...
        _setText : function (value) {
            ...
            // _getText only handles valid / invalid values and
            // prefills, not the helptext
            if (!value && !this._isPassword) {
                // We don't want to handle helptext in password fields,
                // first remove any text
        ...
        _inputWithFrameMarkup : function (out) {
            ...
            if (this._isTextarea) {
                ...
                out.write(['<input class="xTextInputInput" ', Aria.testMode ? ' id="' + this._domId + '_input"' : '',
                        ...
                        (cfg.tabIndex != null ? 'tabindex="' + this._calculateTabIndex() + '" ' : ' '), spellCheck,
                        ' _ariaInput="1"/>'
                // the _ariaInput attribute is present so that
                // pressing
                // ENTER on this widget raises the onSubmit
                // event of
                // the fieldset:
                ].join(''));
            }
        ...
        checkValue : function (arg) {
            ...
            if (!this.controller) {
                // There's no controller so we assume the value to be
                // valid
                return result;
            }
            ...
            if (!report) {
                // No report means that the controller is handling the
                // value asynchronously, consider it as valid
                return result;
            }
            ...
            if (report.errorMessages.length) {
                ...
                // setting invalid text to null means we might add
                // helptext, this prevents any value to be applied
                this.setHelpText(false);
            } else if (report.ok && !performCheckOnly) {
                this.changeProperty("invalidText", null);
                // If I get there there are no errors raised by this
                // check, and there were no errors before, meaning the
                // value is correct, no need to remove the helptext
        ...
        _reactToControllerReport : function (report, arg) {
            ...
            if (report) {
                ...
                if (arg) {
                    resetErrorIfOK = (arg.resetErrorIfOK !== false); // true
                    // by
                    // default
                ...
                // if the validation originated from a validation with
                // delay we
                // do not want to update the input text or
                // value. The value will be set to 'undefined' though
                // when the
                // entry is incorrect
                if (!delayedValidation) {
                    var text = report.text, value = report.value;
                    if (text != null && !this._helpTextSet) { // if
                        // text
                        // was
                        // an empty string (helpTextSet is 'true') do
                        // not update
                        // the display
                ...
                if (this._cfg) {
                    if (report.ok) {
                        ...
                        if (hasFocus && report.matchCorrectValueStart) {
                            // field has the focus and entry could be
                            // correct
                        ...
                        // if the text is incorrect, the bound property
                        // should
                        // be set to 'undefined'
                ...
                if (hasChange && this._cfg) {
                    // When the user modifies the field, we reset the
                    // error
        ...
        _onBoundPropertyChange : function (propertyName, newValue, oldValue) {
            if (propertyName === 'value') {
                ...
                // in case things have changed the field, try to set an
                // helptext
                this.setHelpText(true);
                ...
                // no need to handle combinations of newValue and
                // oldValue being: null, undefined, ''
                // this can happen when invalidText is set from the
                // checkValue below, causing an infinite loop
                ...
                // only update the display value if the old value is not
                // a valid value, or the old value is null
                if (!res || !res.isValid) {
                ...
                if (newValue) {
                    // PTR 04746599: disabling the field or making it
                    // read-only
        ...
        _setState : function () {
            if (this._cfg.disabled) {
                ...
                // PTR 04746599: the disabledFocused state must not be
                // reachable
        ...
        _updateState : function () {
            // PROFILING // var profilingId = this.$startMeasure("update
            // state
            // (TextInput)");
            ...
            if (!(this._cfg.formatError || this._cfg.error)) {
                // remove the error tip when there isn't any error
                // anymore
        ...
        _reactToChange : function () {
            ...
            if (inputElm) {
                this._updateState();
                // sets the readOnly disabled flags in the input
                // according to
        ...
        _dom_onkeydown : function (event) {
            ...
            if (enterPressed) {
                // pressing enter in a field triggers checkValue
                // (necessary
                // especially if doing validation on the
                // onSubmit event of the fieldset, so that the data
                // model is up
                // to date)
                this.checkValue();
            }
        ...
        _dom_onblur : function (event) {
            ...
            if (!this._keepFocus) {
                    ...
                    if (cfg.directOnBlurValidation) {
                        if (cfg.bind) {
                            ...
                            if (bind) {
                                ...
                                // PTR05705466: validateValue could have
                                // triggered widget dispose, need to
                                // re-check
                ...
                // this._hasFocus = false must be after the call of
        ...
        _checkCfgConsistency : function () {
            ...
            if (cfg.autoselect == null) {
                // the default value for autoselect comes from the
                // environment
            ...
            var value = cfg.value;
            if (cfg.invalidText) {
                // Check the field depending on the value and the
                // existing error
                ...
                if (!rep.isValid) {
                    ...
                    // Just to set the this._helpTextSet to true in this
                    // case,
                    ...
                    // for autocomplete, no report is raised.
                    // Asynchronous
                ...
                // if the value is not set (namely it is null,
                // undefined, an
        ...
        setHelpText : function (enable) {
            ...
            // stops if no helptext, or trying to disable an helptext
            // that as
            // not been set
        ...
        focus : function (idArray, fromSelf) {
            ...
            // IE FIX: requires the value to be reset for the cursor to
            // be positioned
        ...
        _autoselect : function () {
            // this._cfg can be null due to the following execution
            // chain: native focus -> native blur -> AutoComplete
            // bound value update in the data model -> bindRefreshTo
            // section refresh
            if (this._firstFocus && this._cfg && this._cfg.autoselect) {
                // this allow to click again and put the cursor at a
                // given position
