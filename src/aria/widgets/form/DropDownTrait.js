...
/**
 * Class whose prototype is intended to be imported for all widgets that use a
 * drop-down popup
 */
Aria.classDefinition({
	...
	$constructor : function() {
		// The purpose of this class is to provide a prototype to be imported,
	...
	$prototype : {
		...
		/**
		 * Raised when the viewport is resized. Needed to fix the position of
		 * the popup after the resize.
		 * 
		 * @param {Object}
		 *            evt
		 */
		...
		_frame_events : function(evt) {
			if (evt.name == "iconMouseDown" && evt.iconName == "dropdown"
					&& !this._cfg.disabled) {
				if (this._hasFocus) {
					this._keepFocus = true;
				}
			} else if (evt.name == "iconClick" && evt.iconName == "dropdown"
					&& !this._cfg.disabled) {
		...
		/**
		 * Internal method called when the popup should be either closed or
		 * opened depending on the state of the controller and whether it is
		 * currently opened or closed. In any case, keep the focus on the field.
		 * Called by the widget button for example. To be overridden.
		 * 
		 * @protected
		 */
		...
		/**
		 * Returns an array of html elements on which it is possible to click
		 * without closing the popup. This array is passed in the popup
		 * configuration as the ignoreClicksOn property. This method can be
		 * overridden if needed. Default implementation returns the dropdown
		...
		_getPopupIgnoreClicksOnDomElts : function() {
			return this._frame && this._frame.getIcon ? [ this._frame
					.getIcon("dropdown") ] : null;
		},
		...
		/**
		 * Internal function to override to render the content of the dropdown
		 * div
		 * 
		 * @protected
		 * @param {aria.templates.MarkupWriter}
		 *            out Markup writer which should receive the content of the
		 *            popup.
		 * @param {Object}
		 *            arg Optional parameters
		 */
		 ...
		 * 
		 * @protected
		 * @param {Object|aria.DomEvent}
		 *            event object containing keyboard event information (at
		 *            least charCode and keyCode properties). This object may be
		 *            or may not be an instance of aria.DomEvent.
		 */
		...
		/**
		 * Get the preferred width for the dropdown popup. This value is part of
		 * the widget configuration. If bigger than 0 the width of the popup
		 * will be fixed and won't be computed automatically.
		 * 
		 * @protected
		 */
		...
		/**
		 * Set the property "popupOpen" in the widget configuration and in the
		 * data model to which it is n=bound
		 * 
		 * @param {Boolean}
		 *            value
		 * @protected
